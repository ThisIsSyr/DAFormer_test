...
def wavelet_transform(x, filters):
    b, c, h, w = x.shape
    pad = (filters.shape[3] // 2 - 1, filters.shape[3] // 2 - 1,
           filters.shape[2] // 2 - 1, filters.shape[2] // 2 - 1)
...
class WTConv2d(nn.Module):
    """
    Wavelet convolution block:
    - 多层小波分解 -> 在小波子带上做 depthwise conv -> 逆小波重建
    - 再加一个 base depthwise conv 的残差
    """
    def __init__(self, in_channels, out_channels,
        ...
        wt_filter, iwt_filter = create_wavelet_filter(
            wt_type, in_channels, in_channels, torch.float32)
        ...
        self.base_conv = nn.Conv2d(
            ...
            padding=pad, stride=1, dilation=1,
        ...
        if self.stride > 1:
            ...
            self.do_stride = lambda x_in: F.conv2d(
                x_in, self.stride_filter, bias=None,
    ...
    def forward(self, x):
        ...
        for i in range(self.wt_levels - 1, -1, -1):
            ...
            curr_x = torch.cat(
                [curr_x_ll.unsqueeze(2), curr_x_h], dim=2)
...
class WaveletPreprocess(nn.Module):
    """
    简单封装：输入/输出形状不变 (B, 3, H, W)，只做一次 WTConv2d
    """
    def __init__(self,
                 in_channels=3,
                 kernel_size=5,
